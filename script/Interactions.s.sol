...
contract FundSubscription is CodeConstants, Script {
    uint256 public constant FUND_AMOUNT = 10e18;
    ...
    function fundSubscription(address vrfCoordinatorV2_5, uint256 subId, address link, address account) public {
        ...
        if (block.chainid == LOCAL_CHAIN_ID) {
            ...
            console.log("Approving LINK for VRF Coordinator...");
            vm.startBroadcast(account);
            LinkToken(link).approve(vrfCoordinatorV2_5, FUND_AMOUNT); // Add this line
            vm.stopBroadcast();

            uint256 allowance = LinkToken(link).allowance(account, vrfCoordinatorV2_5);
            console.log("Allowance for VRF Coordinator after approval:", allowance);

            require(allowance >= FUND_AMOUNT, "Allowance is insufficient for funding!");
